...
Source: repo-wide ponytail-audit, published as GitHub issues [#103](https://github.com/laraxot/<nome repository>/discussions/114).
...
Discussion [#82](https://github.com/laraxot/<nome repository>/discussions/83) established a repository-wide architecture rule: **business logic must live in `QueueableAction` classes (`spatie/laravel-queueable-action`), never in Service classes.** `Modules/Xot/app/Services/` and `Modules/Job/app/Services/` are the only allowed exception (framework wrappers, not business logic).
